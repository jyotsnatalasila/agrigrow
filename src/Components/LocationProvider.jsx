...
export function LocationProvider({ children }) {
  ...
  // PERSIST pickedPosition IN LOCALSTORAGE
  const [pickedPosition, setPickedPosition] = useState(() => {
    try {
      const stored = localStorage.getItem('agrigrowPickedPosition');
      return stored ? JSON.parse(stored) : [22.9734, 78.6569];
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('agrigrowPickedPosition', JSON.stringify(pickedPosition));
    } catch (e) {}
  }, [pickedPosition]);
